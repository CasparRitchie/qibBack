...
app.get('/documents', auth, async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM documents WHERE production_id IN (SELECT id FROM productions WHERE company_id = ?)', [req.user.company_id]);
    connection.release();
    res.json(rows);
  } catch (err) {
    console.error(err);
...
// Download a file, ensuring it belongs to the user's company
app.get('/download/:id', auth, async (req, res) => {
  const documentId = req.params.id;

  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM documents WHERE id = ? AND production_id IN (SELECT id FROM productions WHERE company_id = ?)', [documentId, req.user.company_id]);
    connection.release();
